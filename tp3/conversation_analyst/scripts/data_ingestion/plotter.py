...
def plots(chat_messages, name, analysis_id):
    ...
    colors = {
        "message_risk": "yellow",
        "message_name_count": "blue",
        "message_location_count": "green",
    }
    markers = {
        "message_risk": "circle",
        "message_name_count": "square",
        "message_location_count": "triangle-up",
    }
    ...
    for sender, sender_data in data.items():
        ...
        if name_non_zero_indices.size > 0:
            fig.add_trace(
                go.Scatter(
                    x=timestamps[name_non_zero_indices],
                    y=np.array(sender_data["message_lengths"])[name_non_zero_indices],
                    mode="markers",
                    marker=dict(
                        color=colors["message_name_count"],
                        symbol=markers["message_name_count"],
                    ),
                    name=None,
                    showlegend=False,
                )
        ...
        if location_non_zero_indices.size > 0:
            fig.add_trace(
                go.Scatter(
                    x=timestamps[location_non_zero_indices],
                    y=np.array(sender_data["message_lengths"])[
                        location_non_zero_indices
                    ]
                    + 5,
                    mode="markers",
                    marker=dict(
                        color=colors["message_location_count"],
                        symbol=markers["message_location_count"],
                    ),
                    name=None,
                    showlegend=False,
                )
        ...
        if risk_non_zero_indices.size > 0:
            fig.add_trace(
                go.Scatter(
                    x=timestamps[risk_non_zero_indices],
                    y=np.array(sender_data["message_lengths"])[risk_non_zero_indices]
                    - 5,
                    mode="markers",
                    marker=dict(
                        color=colors["message_risk"], symbol=markers["message_risk"]
                    ),
                    name=None,
                    showlegend=False,
                )
